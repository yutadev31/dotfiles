...
vim.lsp.config("cspell_ls", {
  filetypes = {
    "css", "gitcommit", "go", "html", "javascript", "javascriptreact", "json", "lua", "markdown", "nix", "rust",
    "text", "toml", "typescript", "typescriptreact", "yaml",
  },
})
...
if vim.fn.executable("cspell") == 1 then
  vim.api.nvim_create_user_command("Cspell", function()
    ...
    local result = vim.system({ "cspell", "lint", "--no-progress", "--no-summary", file }, { text = true }):wait()
    local lines = vim.split(result.stdout or "", "\n", { trimempty = true })
    vim.fn.setqflist({}, "r", { title = "cspell", lines = lines, efm = "%f:%l:%c - %m" })
